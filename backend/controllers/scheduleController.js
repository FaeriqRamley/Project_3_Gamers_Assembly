...
module.exports.createTimeslotInvite_post = async (req, res) => {
    ...
    await Schedule.findOneAndUpdate(
        { ownerId: req.body.senderId },
        { $push: { sentNotifications: newInvite._id } }
    );
    await Schedule.findOneAndUpdate(
        { ownerId: req.body.receiverId },
        { $push: { receivedNotifications: newInvite._id } }
    );
...
module.exports.respondTimeslotInvite_put = async (req, res) => {
    ...
    if (!inviteObj) {
        res.json({ status: "failed", message: "invite does not exist" });
    }
    ...
    if (req.params.response === "Decline") {
        ...
        await Schedule.findOneAndUpdate(
            { ownerId: receiverId },
            { $pull: { receivedNotifications: inviteId } }
        );
...
module.exports.removeRespondedTimeslotInvite_delete = async (req, res) => {
    await Schedule.findOneAndUpdate(
        { ownerId: req.body.senderId },
        { $pull: { sentNotifications: req.body.inviteId } }
    );
    await Invite.findByIdAndDelete(req.body.inviteId);
    res.json({
        status: "ok",
        messaged: "Removed invite from db and reference from sender's schedule",
    });
};
